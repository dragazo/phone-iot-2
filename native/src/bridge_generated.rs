...
fn wire_initialize_impl(
    ...
    FLUTTER_RUST_BRIDGE_HANDLER.wrap::<_, _, _, ()>(
        ...
        move || {
            ...
            move |task_callback| Ok(initialize(api_device_id, api_utc_offset_in_seconds))
        },
    )
}
    ...
    fn into_dart(self) -> support::DartAbi {
        match self {
            ...
            Self::GetAccelerometer { key } => {
                vec![23.into_dart(), key.into_into_dart().into_dart()]
            }
            Self::GetLinearAccelerometer { key } => {
                vec![24.into_dart(), key.into_into_dart().into_dart()]
            }
            Self::GetGyroscope { key } => vec![25.into_dart(), key.into_into_dart().into_dart()],
            Self::GetMagnetometer { key } => vec![26.into_dart(), key.into_into_dart().into_dart()],
            Self::GetGravity { key } => vec![27.into_dart(), key.into_into_dart().into_dart()],
            Self::GetPressure { key } => vec![28.into_dart(), key.into_into_dart().into_dart()],
            Self::GetRelativeHumidity { key } => {
                vec![29.into_dart(), key.into_into_dart().into_dart()]
            }
            Self::GetLightLevel { key } => vec![30.into_dart(), key.into_into_dart().into_dart()],
            Self::GetTemperature { key } => vec![31.into_dart(), key.into_into_dart().into_dart()],
            Self::GetFacingDirection { key } => {
                vec![32.into_dart(), key.into_into_dart().into_dart()]
            }
            Self::GetOrientation { key } => vec![33.into_dart(), key.into_into_dart().into_dart()],
